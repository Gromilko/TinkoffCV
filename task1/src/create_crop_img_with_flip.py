...
if __name__ == "__main__":
    ...
    for count, tif_name in enumerate(tif_list):
        ...
        for i in range(50):
            ...
            if np.random.randint(0, 10) == 5:
                ...
                data_test.append(np.flip(im_crop, 0))
                label_test.append(np.flip(ms_crop, 0))
                data_test.append(np.flip(im_crop, 1))
                label_test.append(np.flip(ms_crop, 1))
            else:
                data.append(im_crop)
                label.append(ms_crop)
                data.append(np.flip(im_crop, 0))
                label.append(np.flip(ms_crop, 0))
                data.append(np.flip(im_crop, 1))
                label.append(np.flip(ms_crop, 1))

    label = np.array(label).reshape(int(len(label)), 1, 256, 256)
    data = np.array(data).reshape(int(len(data)), 1, 256, 256)
    np.save('../data/build_train_data_flip_no_road.npy', data)
    np.save('../data/build_train_label_flip_no_road.npy', label)
    ...
    np.save('../data/build_test_data_flip_no_road.npy', data)
    np.save('../data/build_test_label_flip_no_road.npy', label)
